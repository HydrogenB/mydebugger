...
import { PermissionEvent } from '../lib/permissions';
    ...
    <Card className="h-full">
      <div className="p-4 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between">
          <h3 className="font-semibold text-gray-900 dark:text-white">Event Log</h3>
          <div className="flex gap-2">
            <Button variant="ghost" size="sm" onClick={onCopyLog} disabled={events.length === 0}>
              <FiCopy className="w-3 h-3" />
            </Button>
            <Button variant="ghost" size="sm" onClick={onClearLog} disabled={events.length === 0}>
              ...
              <div key={event.id} className="flex items-start gap-3 text-sm">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-gray-900 dark:text-white">{event.permissionName}</span>
                    <span className={`text-xs ${getEventColor(event.action)}`}>{event.action}</span>
                  </div>
                  {event.details && (
                    <p className="text-gray-600 dark:text-gray-400 truncate">{event.details}</p>
                  )}
                </div>
